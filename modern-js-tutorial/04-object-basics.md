...
John으로 들어오는 참조가 전부 사라진다. **외부에서 들어오는 참조**만이 도달 가능성에 영향을 준다. John이 wife를 참조하고 있어도(나가는 참조), 아무도 John을 참조하지 않으면 삭제된다.
...
`family`를 `null`로 바꾸면 루트와의 연결이 끊긴다. John과 Ann이 서로를 참조하고 있어도, 루트에서 도달할 수 없으면 **둘 다 삭제**된다.
