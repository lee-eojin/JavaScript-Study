...
## 17-arrow-functions-basics: 화살표 함수 기본

함수 표현식보다 더 짧게 함수를 만드는 방법이다.
...
중괄호로 감싸고, `return`을 명시해야 한다.

```js
let sum = (a, b) => {
  let result = a + b;
  return result;  // 중괄호 쓰면 return 필수
};
```
...
>
> ```js
> let double = n => n * 2;  // return n * 2 와 같음
> ```
>
> **중괄호 있으면 return 필수**
>
> ```js
> let double = n => { return n * 2; };  // return 없으면 undefined
> ```

### 콜백에서 진가 발휘

화살표 함수는 콜백으로 넘길 때 진짜 편하다.
...
> - 함수 표현식: 거의 안 씀
>
> 콜백은 거의 다 화살표 함수로 쓴다. 함수 선언문은 메인 함수나 호이스팅 필요할 때 쓴다.

### 주의: 아직 다 배운 게 아님

화살표 함수는 단순히 "짧은 문법"만 있는 게 아니다. 일반 함수와 다른 특징이 있다:

- `this`를 바인딩하지 않음
- `arguments` 객체가 없음
- 생성자로 사용 불가 (`new` 안 됨)

이건 나중에 더 배운다. 지금은 **짧게 쓰는 문법**이라고만 알면 됨.

### 정리

| 형태 | 문법 | return |
|------|------|--------|
| 본문 한 줄 | `() => expression` | 자동 |
| 본문 여러 줄 | `() => { ... }` | 명시 필요 |
