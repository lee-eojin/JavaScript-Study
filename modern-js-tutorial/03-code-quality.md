...
전부 뒤집으면 좋은 코드 규칙이 된다:

| 닌자 코드 (하지 마라) | 좋은 코드 (이렇게 해라) |
|------------------------|--------------------------|
| 한 글자 변수, 약어 | 의미 있는 이름 |
| `data`, `value` 같은 불용어 | 구체적인 이름 |
| 같은 동작에 다른 이름 | 접두어 통일 |
| 변수 재사용 | 새 변수 선언 |
| 외부 변수 가리기 | 다른 이름 사용 |
| 부작용 있는 함수 | 이름대로만 동작 |
...
## 05-testing-mocha: 테스트 자동화와 Mocha

### 테스트는 왜 해야 하는가?

함수를 만들고 수동으로 결과를 확인하는 건 불완전하다. `f(1)`을 고쳐서 통과시켰는데, 그러다 `f(2)`가 깨질 수 있다. 코드를 수정할 때마다 모든 케이스를 수동으로 확인하는 건 현실적으로 불가능하다.

테스트 자동화는 이 문제를 해결한다. 테스트 코드를 한 번 작성해두면, 코드가 바뀔 때마다 자동으로 모든 케이스를 검증할 수 있다.

### BDD (Behavior Driven Development)

> **BDD (Behavior Driven Development)**
>
> 테스트, 문서, 예시를 한데 모아놓은 개발 방법론이다.
> 코드를 작성하기 전에 "이 함수가 어떤 동작을 해야 하는지"를 먼저 정의하고,
> 그 정의(스펙)에 맞게 코드를 구현하는 방식이다.

### 명세서 (스펙)

코드가 무슨 일을 하는지 자연어로 표현한 산출물이다.
...
> | 키워드 | 역할 |
> |--------|------|
> | `describe("제목", function)` | 테스트 그룹. "뭘 테스트하는지" |
> | `it("설명", function)` | 개별 테스트. "어떤 동작을 해야 하는지" |
> | `assert.equal(값1, 값2)` | 기대값 비교. 다르면 에러 |

### 개발 순서

1. 스펙 초안 작성 (기본 테스트 포함)
2. 스펙을 보고 코드 작성
3. 테스트 프레임워크로 스펙 실행
4. 에러 없을 때까지 코드 수정
5. 새로운 유스케이스 추가 (테스트 실패 시작)
6. 3번으로 돌아가서 반복

실패하는 테스트를 먼저 추가하고, 통과하도록 코드를 개선하는 게 BDD의 핵심이다.

### 스펙 실행에 쓰이는 라이브러리

| 라이브러리 | 역할 |
|------------|------|
| [Mocha](http://mochajs.org/) | 테스트 프레임워크. `describe`, `it` 등 제공 |
| [Chai](http://chaijs.com) | assertion 라이브러리. `assert.equal` 등 제공 |
| [Sinon](http://sinonjs.org/) | 함수 모방(mock). 내장 함수 등을 흉내 |

### 테스트 하나에선 한 가지만 확인

```js
// 나쁜 예 - 첫 번째 assert 실패하면 두 번째 결과를 모름
it("주어진 숫자의 n 제곱", function() {
  assert.equal(pow(2, 3), 8);
...
it("3을 네 번 곱하면 81", function() {
  assert.equal(pow(3, 4), 81);
});
```

`assert`에서 에러가 발생하면 `it` 블록이 즉시 종료되기 때문에, 하나의 `it`에 여러 `assert`를 넣으면 첫 번째가 실패했을 때 나머지 결과를 알 수 없다.

### for문으로 테스트 자동 생성

수동으로 `it` 블록을 반복 작성하는 대신, 헬퍼 함수와 반복문으로 자동 생성할 수 있다.
...
describe("pow", function() {
  ...
  function makeTest(x) {
    let expected = x * x * x;
    it(`${x}을/를 세 번 곱하면 ${expected}`, function() {
...
`describe` 안에 `describe`를 넣어서 테스트를 그룹화할 수 있다.
...
`makeTest`는 안쪽 `describe` 블록 안에서 선언됐기 때문에, 바깥 최상위 레벨에서는 접근할 수 없다. 스코프가 분리되니까 나중에 최상위에 새 테스트를 추가해도 서로 안 건드린다.

### before/after와 beforeEach/afterEach

| 함수 | 실행 시점 |
|------|-----------|
| `before` | 전체 테스트 시작 전 (1회) |
| `after` | 전체 테스트 종료 후 (1회) |
| `beforeEach` | 매 `it` 실행 전 |
| `afterEach` | 매 `it` 실행 후 |

초기화 용도로 쓴다. 카운터를 0으로 리셋하거나, 테스트마다 필요한 준비 작업이 있을 때 사용한다.
...
> | 메서드 | 역할 |
> |--------|------|
> | `assert.equal(a, b)` | `a == b` 동등 비교 |
> | `assert.strictEqual(a, b)` | `a === b` 일치 비교 |
> | `assert.notEqual(a, b)` | `a != b` |
> | `assert.isTrue(value)` | `value === true` |
> | `assert.isFalse(value)` | `value === false` |
> | `assert.isNaN(value)` | NaN인지 확인 |

### 스펙의 세 가지 용도

1. **테스트** - 함수가 의도한 대로 동작하는지 보장
2. **문서** - `describe`와 `it`의 설명이 곧 문서
3. **예시** - 실제 동작하는 사용법 예시

스펙이 있으면 코드를 안전하게 수정하거나 개선할 수 있다. 대규모 프로젝트에서 함수 하나를 이곳저곳에서 쓰는데, 수동으로 전부 확인하는 건 불가능하기 때문이다. 잘 테스트된 코드는 더 나은 아키텍처를 만든다.

---

## 06-polyfills: 폴리필

자바스크립트는 끊임없이 진화하는 언어다. 새로운 기능이 계속 추가되는데, 모든 엔진이 최신 기능을 전부 지원하지는 않는다. 이 간극을 메우는 도구가 트랜스파일러와 폴리필이다.

### 트랜스파일러

새로운 **문법**으로 작성한 코드를 옛날 문법으로 변환해주는 도구다.

```js
// 내가 쓴 코드 (최신 문법)
const greet = (name) => `Hello ${name}`;

// 트랜스파일러가 변환한 코드 (옛날 문법)
...
> **바벨 (Babel)**
>
> 가장 유명한 자바스크립트 트랜스파일러다.
> 트랜스파일러가 역할이라면, 바벨은 그 역할을 하는 제품 이름이다.
> ESLint가 린터의 제품명인 것처럼.

### 폴리필

트랜스파일러가 "문법"을 바꿔주는 거라면, 폴리필은 없는 **기능/함수**를 직접 구현해서 넣는 거다. 구멍(없는 기능)을 메꿔준다(fill)는 의미에서 polyfill이다.

```js
// Array.includes()가 없는 옛날 브라우저를 위한 폴리필
...
> | | 트랜스파일러 | 폴리필 |
> |---|---|---|
> | 대상 | 새로운 **문법** (화살표 함수, `const` 등) | 새로운 **기능/함수** (`includes`, `Promise` 등) |
> | 방식 | 코드를 변환 | 없는 함수를 직접 구현해서 추가 |

주목할 만한 폴리필:
- [core-js](https://github.com/zloirock/core-js) - 다양한 폴리필 제공. 특정 기능만 골라 쓸 수 있다
- [polyfill.io](http://polyfill.io) - 브라우저에 따라 필요한 폴리필을 자동 제공

### 웹팩

> **웹팩 (Webpack)**
>
> 빌드 도구다. 프로젝트 파일이 수십, 수백 개가 되면 이걸 하나로 묶어야 한다.
> 묶는 과정에서 바벨(트랜스파일러)도 자동으로 돌려주고, 코드 압축도 해준다.
> 코드 수정할 때마다 알아서 다시 빌드해주니까 수동으로 할 필요가 없다.

모던 자바스크립트로 개발하려면 트랜스파일러와 폴리필은 필수다. 지금 당장 직접 쓸 일은 없지만, 나중에 프로젝트 세팅할 때 다시 만나게 된다.
